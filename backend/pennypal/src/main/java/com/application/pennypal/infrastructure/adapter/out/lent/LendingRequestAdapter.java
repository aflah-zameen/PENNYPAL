...
public class LendingRequestAdapter implements LendingRequestRepositoryPort {
    ...
    @Override
    public List<LendingRequest> getAllLendingRequestsSent(String userId) {
        return lendingRequestRepository.findByRequestedByOrderByCreatedAtDesc(userId).stream()
                .map(LendJpaMapper::toDomain)
                .toList();
    }

    @Override
    public List<LendingRequest> getAllLendingRequestsReceived(String userId) {
        return lendingRequestRepository.findByRequestedToOrderByCreatedAtDesc(userId).stream()
                .map(LendJpaMapper::toDomain)
                .toList();
    }

    @Override
    public long getLendingRequestSentCount(String userId) {
        return lendingRequestRepository.countByRequestedByAndStatus(userId,LendingRequestStatus.PENDING);
    }
