...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<UserDomainDTO>> login(@Valid @RequestBody LoginRequest request, HttpServletRequest httpServletRequest, HttpServletResponse response){
        ...
        LoginResponseDTO loginResponseDTO = loginUser.execute(request.getEmail(), request.getPassword(),httpServletRequest.getRemoteAddr());
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        setAccessTokenCookie(loginResponseDTO.accessToken(), response);
        setRefreshTokenCookie(loginResponseDTO.refreshToken(), response);
        return ResponseEntity.ok(new LoginResponse(true,loginResponseDTO.user(),"Successfully Login"));
    }
